...
def naverindoorinit(opt):
    global dataset,datasetroot
    parsing={
        'g1':"GangnamStation/B1/release",
        'g2':"GangnamStation/B2/release",
        'h':"GangnamStation/B1/release",
        'h':"GangnamStation/B1/release",
        'h':"GangnamStation/B1/release",
    }
    dataset=opt.naver
    datasetroot=parsing[opt.naver]
...
def get_whole_training_set(onlyDB=False):
    structFile = join(root_dir, datasetroot, "map_plus_val")
...
def get_whole_test_set():
    structFile = join(root_dir, datasetroot, "map_plus_test")
    return WholeDatasetFromStruct(structFile,
                                  whichSet="test",
                                  input_transform=input_transform())
...
def parse_dbStruct(path,whichSet="train"):
    tar_handlers = get_all_tar_handlers(path)
    kapture_data = kapture_from_dir(path, tar_handlers=tar_handlers)
    root = get_image_fullpath(path, image_filename=None)

    dbImage=[]
    utmDb=[]
    qImage = []
    utmQ = []
    for k1 in kapture_data.records_camera.keys():
        for k2 in kapture_data.records_camera[k1].keys():
            if "images" in kapture_data.records_camera[k1,k2]:
                dbImage.append(join(root,kapture_data.records_camera[k1,k2]))
                utmDb.append(kapture_data.trajectories[k1,k2].t)
            elif "galaxy" in kapture_data.records_camera[k1,k2]:
                qImage.append(join(root,kapture_data.records_camera[k1,k2]))
                utmQ.append(kapture_data.trajectories[k1,k2].t)
                
    utmDb=np.squeeze(np.stack(utmDb,0))
    utmQ=np.squeeze(np.stack(utmQ,0))
    numDb = len(dbImage)
    numQ = len(qImage)

    posDistThr = 100
    posDistSqThr = 10000
    nonTrivPosDistSqThr = 1600
...
class WholeDatasetFromStruct(data.Dataset):
    def __init__(self, structFile, whichSet, input_transform=None, onlyDB=False):
        super().__init__()

        self.input_transform = input_transform

        self.dbStruct = parse_dbStruct(structFile,whichSet=whichSet)
...
class QueryDatasetFromStruct(data.Dataset):
    def __init__(self, structFile, whichSet, nNegSample=1000, nNeg=10, margin=0.1, input_transform=None):
        super().__init__()
    ...
    def __getitem__(self, index):
        ...
        with h5py.File(self.cache, mode='r') as h5: 
            ...
            negFeat = h5feat[negSample.astype(np.int32).tolist()]
            knn.fit(negFeat)
